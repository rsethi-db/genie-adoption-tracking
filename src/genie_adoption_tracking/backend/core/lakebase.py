...
def create_db_engine(db_config: DatabaseConfig, ws: WorkspaceClient) -> Engine:
    ...
    # Lakebase sits behind a connection pooler that does not support psycopg3's
    # server-side prepared statements — leaving them on surfaces as
    # ResourceClosedError on the second use of a query. Disable them in both dev
    # (tunnel to the same pooler) and prod.
